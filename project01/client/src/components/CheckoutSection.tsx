...
export function CheckoutSection() {
  ...
  const getUserInformation = useCallback(async () => {
    if (email) {
      try {
        const res: AxiosResponse<UserInterface> = await api.get(`${SERVER}/users/${email}`);
  ...
  async function updateUser(user: UserInterface) {
    await api.put(`${SERVER}/users/${user._id}`, user);
  }
  ...
  async function onSubmit(data: z.infer<typeof FormSchema>) {
    ...
    if (email) {
      ...
      const user = (await api.get(`${SERVER}/users/${email}`)).data as UserInterface;
      if (!user) {
